...
public class ArenaCommand implements CommandExecutor {
    ...
    private static final String ADMIN_PERMISSION = "combatlog.bpvp.admin";

    private final CombatLogPlugin plugin;

    public ArenaCommand(CombatLogPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage("§c/bpvp setkit <arena> <kit> §7- kit startowy (nazwa z /kit create)");
    ...
    private boolean handleSetKit(CommandSender sender, String[] args) {
        if (args.length < 3) {
            sender.sendMessage("§cUżycie: /bpvp setkit <arena> <kit>");
            return true;
        }
        String name = args[1];
        if (!requireArena(sender, name)) {
        ...
        if (!plugin.getKits().exists(kitName)) {
            sender.sendMessage("§cKit '" + kitName + "' nie istnieje (najpierw /kit create " + kitName + ").");
            return true;
        }
        plugin.getArenas().setKit(name, kitName);
        sender.sendMessage("§aUstawiono kit startowy '" + kitName + "' dla areny '" + name + "'.");
        return true;
    }
